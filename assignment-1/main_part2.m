...
function [] = colour_spaces(im_name, colour_space)
    ...
    if strcmp(colour_space, 'rgb_col_space')
        [r, g, b] = calc_rgb_col_space(R, G, B);
        show_first_channel(im_zeros, r, 'first channel rgb colour space');
        show_second_channel(im_zeros, g, 'second channel rgb colour space'); 
        show_third_channel(im_zeros, b, 'third channel rgb colour space');
    end
